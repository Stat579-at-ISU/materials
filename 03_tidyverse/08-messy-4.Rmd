...
The Lahman database consists of `r nrow(LahmanData)` data frames that are linked by `playerID`. 


This is clean, but not very readable. 

The `People` table includes names and other attributes for each player (and coaches, managers, ...)
...
Who was inducted in the Hall of Fame in 2022?

```{r warning = FALSE, fig.height = 3.5}
HallOfFame %>% filter(yearID==2022) 
```
...
- How often was Sammy Sosa on the HoF ballot? 
