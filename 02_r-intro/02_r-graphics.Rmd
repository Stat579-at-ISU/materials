...
- Wildly popular package for statistical graphics: millions of downloads per month

```{r, echo=FALSE, cache = TRUE, fig.height = 5}
library(dlstats)
library(ggplot2)

dframe <- cran_stats("ggplot2", use_cache = FALSE)
ggplot(data = dframe[-nrow(dframe),], aes(x = end, y = downloads/1000000)) + geom_point() + geom_line() +
  ylab("Number of monthly downloads (in millions)") + xlab("Date") +
